...
int LCSubStr(string X, string Y, int m, int n)
{
    ...
    for (int i=0; i<=m; i++)
    {
        for (int j=0; j<=n; j++)
        {
            ...
            else if (X[i-1] == Y[j-1] || X[i-1]==Y[j-1]+32 || X[i-1]+32==Y[j-1])
            {
...
int main()
{
  ...
  for(int i=0;i<13;i++)
  {
    ...
    while (getline(filin_output[i], line))
    {
      ...
      /*		int f=0;
            ...
            while (getline(ss, item,' ')) 
            {
            int len=LCSubStr(item,"place",item.size(),5);
      //			cout<<item<<endl;
      if(len==5)
      {
      f++;
  ...
  /*
     ...
     {
     int max=0;
     final.push_back("0000000000000000000");
     for(int j=0;j<rec_out[i].size();j++)
     {
     int c=0;
     for(int k=0;k<19;k++)
  ...
  while (getline(input, line))
  {
    ...
    /*	int sum=0;
        ...
        for(int p=0;p<19;p++)
        {
        if(out1[p]==1 && final[m][p]=='1')
        {
        sum+=1;
        }
        }
        answer.push_back(make_pair(sum,m));
        }
        sort(answer.begin(),answer.end());
        cout<<answer[answer.size()-1].first<<" "<<refer[answer[answer.size()-1].second]<<endl;
        cout<<answer[answer.size()-2].first<<" "<<refer[answer[answer.size()-2].second]<<endl;
        */
  }
